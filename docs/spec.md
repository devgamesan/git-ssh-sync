...
GitHub Actions CI と同じ依存関係同期、lint、format、テスト確認は、ローカルで以下を実行する。
...
現時点の Ruff 対象は Python ソースとテストに限定する。将来ドキュメント形式に対応した tooling を導入する場合は、ローカル確認と CI の両方に docs を追加する。
